...
2. Register the client's intake: business type, target public function,
   prior filing history in Iceland if any.
3. Run the advisor in read-only mode against Útboðsvefur.is.
4. Compare the checklist against the client's current documentation
   (Fyrirtækjaskrá registration status).
...
- named referral relationship with Icelandic-licensed counsel or a
...
a working referral relationship with Icelandic-licensed counsel or a
registered agent for whatever licensed representation the law of
Iceland requires for actual public-procurement filings.
